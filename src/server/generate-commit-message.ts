...
export function summarizeFailures(
	failures: Array<{ provider: 'claude' | 'codex'; reason: string }>,
) {
...
export function sanitizeSubject(value: unknown): string | null {
...
export function fallbackSubject(files: CommitMessageFile[]) {
	if (files.length === 1) {
			...
			parse: (value) => {
				const output =
					value && typeof value === 'object'
						? (value as { subject?: unknown; body?: unknown })
						: {};
