...
function normalizedReadBlocks(
	value: unknown,
): Array<ReadStructuredTextBlock | ReadStructuredImageBlock> {
	const blocks =
		value &&
		typeof value === 'object' &&
		'content' in value &&
...
export function hydrateToolResult(
	...
	switch (tool.toolKind) {
		...
		case 'read_file': {
			...
			const blocks = normalizedReadBlocks(parsed);
			if (blocks.length > 0) {
				return {
			...
			return {
				content:
					typeof record?.content === 'string' ? record.content : JSON.stringify(parsed, null, 2),
			} satisfies ReadFileToolResult;
		}
		default:
			return parsed;
